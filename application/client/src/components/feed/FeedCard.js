    ...
    return (
      <div className="card m-4">
        <div className="card-header d-flex justify-content-center flex-wrap">
          <h2 class="title-font">{recipe.title}</h2> <span className="p-2 ml-2"> by <Link to={`/profile/${recipe.user_id.username}`}>{recipe.user_id.username}</Link></span>
        </div>
        <div className="card-body d-flex flex-md-row flex-column">
          <div className="col-md-4 col-12" style={{ maxWidth: "300px", margin: "auto" }}>
            <img class="img-fluid" src={recipe.img_url} alt={recipe.title} />
          </div>
